...
class Arm:
    ...
    def success_rate_so_far(self):
        denominator, numerator = self.simple_score_from_my_pulls()
...
def random_agent(observation, configuration):
    ...
    """
    # If my opponent repeated itself, copy it
    if len(allActions) >= 2:
    ...
    # At this stage, I have selected everything at least once. My as well
    # try to pick the best one somehow.

    best_arm_so_far = max(list([(a.success_rate_so_far(), a.i) for a in arms]))
    return best_arm_so_far[1]
